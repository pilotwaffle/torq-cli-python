...
def test_auth_status_cli_reports_all_six_and_nonzero(capsys) -> None:
    ...
    assert set(output["providers"]) == {"claude", "codex", "qwen", "kimi", "zai", "deepseek"}
    assert code == output["exit_code"] == 3
    assert all(row["state"] == "unavailable" for row in output["providers"].values())
    assert all(
        row["resolved_model_identity"] == "unattestable"
        for row in output["providers"].values()
    )
...
def test_harness_inspect_cli_flags_mismatch_and_unattestable(tmp_path, capsys) -> None:
    ...
    expected.write_text(json.dumps({"g1d": ["claude", "fable-5"], "g2a": ["codex", "gpt-5.5"]}), encoding="utf-8")
