...
_DECISION_FLAGS = {
    ...
    "codex": "codex_direct_api_primary",
    "qwen": "qwen_anthropic_primary",
...
def validate_provider_matrix(matrix: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    try:
        matrix_date = date.fromisoformat(observed_at) if isinstance(observed_at, str) else None
    except ValueError:
        matrix_date = None
    if matrix_date is None:
        ...
        if not isinstance(surfaces, Mapping) or set(surfaces) != set(REQUIRED_SURFACES):
            ...
            for surface_name, cell in surfaces.items():
                if (
                    ...
                    continue
                try:
                    cell_date = date.fromisoformat(str(cell.get("observed_at")))
                except ValueError:
                    errors.append(f"{name}:{surface_name}:cell_invalid")
                else:
                    if matrix_date is None or cell_date > matrix_date:
                        errors.append(f"{name}:{surface_name}:cell_invalid")
        tos = provider["tos"]
        if (
            ...
            try:
                tos_date = date.fromisoformat(str(tos.get("checked_at")))
            except ValueError:
                errors.append(f"{name}:tos_invalid")
            else:
                if matrix_date is None or tos_date > matrix_date:
                    errors.append(f"{name}:tos_invalid")
